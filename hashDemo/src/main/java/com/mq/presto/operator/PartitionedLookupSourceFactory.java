...
import com.facebook.presto.operator.TrackingLookupSourceSupplier;
        ...
        implements LookupSourceFactory
{
    ...
    @GuardedBy("lock")
    private final Map<Integer, com.mq.presto.operator.SpilledLookupSourceHandle> spilledPartitions = new HashMap<>();
...
//                completed = (partitionsSet == concurrency);
    ...
    private com.mq.presto.operator.SpilledLookupSourceHandle getSpilledLookupSourceHandle(int partitionNumber)
    {
        lock.readLock().lock();
        try {
    ...
    public void destroy()
    {
        ...
        try {
            freePartitions();
            spilledPartitions.values().forEach(com.mq.presto.operator.SpilledLookupSourceHandle::dispose);
